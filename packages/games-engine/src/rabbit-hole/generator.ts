import type { Difficulty, RabbitHoleLink, RabbitHolePuzzle } from "@gentle-stream/domain/games/types";

interface RabbitHoleTemplate {
  topic: string;
  mission: string;
  starterArticle: string;
  links: Array<Omit<RabbitHoleLink, "depth">>;
}
...
export function generateRabbitHole(difficulty: Difficulty, seed: string): RabbitHolePuzzle {
  ...
  const links: RabbitHoleLink[] = template.links.map((link, index) => ({
    ...link,
